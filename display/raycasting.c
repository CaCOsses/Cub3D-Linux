...
int	raycasting(t_display *display) // Envía un rayo del jugador (para cada píxel de línea vertical)
{
...
int	sendrayon(t_display *display) // Cálculo de coordenadas de la intersección con el siguiente cuadro (deltadista)
{
...
int	findwall(t_display *display) // bucle en cada caja hasta que encuentre una pared
{
	int hit;
...
int	findtexturenum(t_display *display) // Busque la textura correspondiente a la pared encontrada
{
	if (display->side == 0)
	{
		if (display->d.x >= 0)
...
{
	if (display->side == 0)                                      // perwalldist = distancia final entre el trabajo y el muro encontrado (para un radio)
	...
	display->lineheight = (int)(display->r2 / display->perwalldist);     // Cálculo de la altura de la pared (cuanto más perwalldist, más cerca será la pared> Raycasting)
