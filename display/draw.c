...
int		put_pxl(t_display *display, int x, int y, unsigned int c)
{
	...
	dest = display->pxl + display->s_line * y + x * (display->bpp / 8); // Modificación del color del valor correspondiente al píxel en la dirección MLX
	*(unsigned int *)dest = c;
	return (0);
}
...
int		draw_line(t_display *display, int x)
{
	...
	y = -1;
	while (++y < display->startline)  // Dibujo del techo de color RGB
		put_pxl(display, x, y,
	...
	while (y < display->endline)
	{
		...
		display->color = *(unsigned int *)(display->ptr[display->textnum] +        // recuperación del color de la textura correspondiente (text_x / tex_y)
